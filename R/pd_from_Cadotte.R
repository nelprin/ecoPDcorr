...
setGeneric("pd_from_Cadotte", function(x, ...) {
    standardGeneric("pd_from_Cadotte")
})

setMethod("pd_from_Cadotte", signature(x="phylo4"),
...
setMethod("pd_from_Cadotte", signature(x="phylo4d"), function(x,
  method=c("traditional", "polytomy", "yule"), ...) {
    phyc <- phylo4com(x, ...)
    pd_from_Cadotte(phyc, method=method)
})

setMethod("pd_from_Cadotte", signature(x="phylo4com"), function(x,
  method=c("traditional", "polytomy", "yule")) {
    ...
    if (method %in% c("polytomy", "yule")) {
        ...
        res <- sapply(communities(x), function(community) {
            ...
            pd_from_Cadotte(extractTree(weightByAbund(phyd, n, method)))
        })
    } else if (method == "traditional") {
        comms <- x@metadata$comms
        if (is.null(comms)) {
            return(.pd_from_Cadotte(extractTree(x), method))
        }
        subtrees <- x@subtrees[as.character(comms)]
        res <- sapply(subtrees, pd_from_Cadotte)[as.character(comms)]
